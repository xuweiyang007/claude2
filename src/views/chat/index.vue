...
const url = "https://a87ndw.laf.dev/claude-api";
...
async function onConversation() {
  ...
  parentMessageId.value = localStorage.getItem("parentId") as string;
  ...
  try {
    ...
    const fetchChatAPIOnce = async () => {
      await axios({
        url,
        method: "post",
        data: { question: message, conversationId: parentMessageId.value },
        responseType: "text",
        headers: {
        ...
        onDownloadProgress: ({ event }) => {
          ...
          if (parts[1]) {
            localStorage.setItem("parentId", parts[1]);
          }
